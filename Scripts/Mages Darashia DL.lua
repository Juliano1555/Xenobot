...
ProductBP = "yellow backpack" --creature products
...
function onWalkerSelectLabel(labelName)
	...
	elseif labelName == "TalkWithBank" then
		...
		local requiredMoneySD = math.max((RequiredSD - Self.ItemCount(SDID)) * SDPrice, 0)
		local requiredMoneyAmmo = math.max((RequiredAmmo - Self.ItemCount(AmmoID)) * AmmoPrice, 0)
		local total = requiredMoneyManas + requiredMoneySD + requiredMoneyAmmo
		Walker.Stop()
		Self.SayToNpc("hi")
	...
	elseif labelName == "BuyPotions" then
		...
		local requiredSDs = math.max(RequiredSD - Self.ItemCount(SDID), 0)
		if requiredManas > 0 or requiredSDs > 0 then
			Walker.Stop()
			Self.SayToNpc("hi")
	...
	elseif labelName == "BuyAmmo" then
		local requiredAmmo = math.max(RequiredAmmo - Self.ItemCount(AmmoID), 0)
		if requiredAmmo > 0 then
			Walker.Stop()
			Self.SayToNpc("hi")
			sleep(math.random(1000, 1500))
			Self.SayToNpc("trade")
			sleep(math.random(500, 800))
			Self.ShopBuyItem(AmmoID, requiredAmmo)
			sleep(math.random(600, 1000))
		end
		Walker.Start()
	...
	elseif labelName == "Check" then
		Walker.Stop()
		if Self.ItemCount(AmmoID) <= AmmoToLeave or Self.ItemCount(ManaID) <= ManasToLeave or Self.ItemCount(SDID) <= SDsToLeave or Self.Cap() <= CapToLeave or Self.Stamina() <= StaminaToLeave then
			Walker.Goto("Leave")
		else
			Walker.Goto("Hunt")
		end
