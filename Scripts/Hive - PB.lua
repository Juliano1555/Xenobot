...
function onWalkerSelectLabel(labelName)
		...
		if requiredManas > 0 or requiredHealths > 0 then
			...
			if requiredManas > 0 then
				...
				repeat
					...
					wait(1000)
				until Self.ItemCount(ManaID) >= requiredManas or (os.time() - started) >= 10
			end
			if requiredHealths > 0 then
				...
				repeat
					...
					wait(1000)
				until Self.ItemCount(HealthID) >= requiredHealths or (os.time() - started) >= 10
			end
	...
	elseif labelName == "Check" then
		Walker.Stop()
		if Self.ItemCount(ManaID) <= ManasToLeave or Self.ItemCount(HealthID) <= HealthsToLeave then
			gotoLabel("Leave")
		else
			gotoLabel("Hunt")
		end
